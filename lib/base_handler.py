...
class BaseHandler(object):
  ...
  # memory.use_heirarchy=1 and memory.move_charge_at_immigrate=3 if the cgroup is under the memory
  # subsystem.
  def config_base_cgroup(self, cgroup_node):
    ...
    if cgroup_node.controller_type == 'memory':
      self.__logger.info('Setting memory.use_heirarchy=1 on {0}'.format(cgroup_node.full_path))
      cgroup_node.controller.use_heirarchy = 1
      self.__logger.info('Setting memory.move_charge_at_immigrate=3 on {0}'.format(cgroup_node.full_path))
      cgroup_node.controller.move_charge_at_immigrate = 3
  ...
  # cgroups_perms parameter provided to __init__(), and sets memory.use_heirarchy=1 and
